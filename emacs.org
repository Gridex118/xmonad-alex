...
#+end_src
To visit files without saving to recentf,
#+begin_src emacs-lisp
...
#+end_src
Remove angle bracket pairing for =Javascript= mode
#+begin_src emacs-lisp
  (mapc (lambda (hook)
          (add-hook hook
                    (lambda ()
...
#+end_src
[[https://github.com/minad/cape][Cape]] for additional capf backends
#+begin_src emacs-lisp
  (use-package cape
    :ensure t
    :init
...
#+end_src
=consult-outline= should be key bound in Org files
#+begin_src emacs-lisp
  (add-hook 'org-mode-hook
  	  (lambda()
  	    (local-set-key (kbd "C-c c o") 'consult-outline)))
#+end_src
When =persp-mode= is available, augment =consult= with the selected perspective's buffer list.
#+begin_src emacs-lisp
...
*** Git
[[https://docs.magit.vc/devel/magit/Options-used-by-commit-commands.html#index-magit_002dcommit_002ddiff_002dinhibit_002dsame_002dwindow][Magit]] provides an option to force [[*Prevent Layout Restructuring By emacs][layout restructuring]] while commiting.
#+begin_src emacs-lisp
  (use-package magit
    :ensure t
    :defer  t
...
[[https://github.com/emacsorphanage/git-gutter][git-gutter]] and [[https://github.com/syohex/emacs-git-gutter-fringe][git-gutter-fringe]] Provide fringe git changes. It appears that both are required to be present for multiline, connected markings to function
#+begin_src emacs-lisp
...
*** Prevent Layout Restructuring By emacs
(Snippet copied from [[https://github.com/nex3/perspective-el#some-musings-on-emacs-window-layouts][perspective-el's github page]])
...
*** Persp Mode
[[https://github.com/Bad-ptr/persp-mode.el][GitHub]]
For saving and restoring 'workspaces'. /Do not save =magit=, =sudo= buffers, or buffers starting with "*"./
#+begin_src emacs-lisp
...
Options that apparently improve performance
#+begin_src emacs-lisp
...
Use the new direct async process options
#+begin_src emacs-lisp
...
*** Agenda
Keybinding for agenda list, if inside an org buffer
#+begin_src emacs-lisp
  (add-hook 'org-mode-hook
            (lambda ()
...
*** Direnv
Use the 'buffer local' fork, =envrc=
#+begin_src emacs-lisp
  (use-package envrc
    :ensure t
    :hook
...
**** Indentations
JSON and JSX files should set JS indent to 2
#+begin_src emacs-lisp
...
**** Generate a Simple Compile Command
#+begin_src emacs-lisp
...
#+end_src
**** Templates That aren't Yasnippets
#+begin_src emacs-lisp
...
*** Open Images in Feh (Fullscreened)
#+begin_src emacs-lisp
